...
<body style='font-family: Arial, sans-serif; background:#f6f8fa; padding:24px;'>
  <div style='max-width:560px; margin:auto; background:#ffffff; border-radius:8px; padding:32px; box-shadow:0 2px 8px rgba(0,0,0,0.08);'>
    ...
    <p style='color:#666; font-size:12px; margin-top:32px;'>Đây là email tự động, vui lòng không trả lời.</p>
        ...
        public (string subject, string htmlBody) BuildAccountApprovedEmail(string fullName, string roleName, string email)
        {
            var subject = "Tài khoản của bạn đã được phê duyệt - CMMS Smart Farm";
            var htmlBody = $@"
<!DOCTYPE html>
<html>
<head><meta charset='UTF-8'></head>
<body style='font-family: Arial, sans-serif; background:#f6f8fa; padding:24px;'>
  <div style='max-width:560px; margin:auto; background:#ffffff; border-radius:8px; padding:32px; box-shadow:0 2px 8px rgba(0,0,0,0.08);'>
    <h2 style='color:#2e7d32; margin-top:0;'>Chào mừng {fullName}!</h2>
    <p>Tài khoản của bạn trên hệ thống <strong>CMMS Smart Farm</strong> đã được phê duyệt với vai trò <strong>{roleName}</strong>.</p>
    <table style='width:100%; margin:16px 0; border-collapse:collapse;'>
      <tr><td style='padding:8px; background:#f1f3f5;'><strong>Email đăng nhập</strong></td><td style='padding:8px;'>{email}</td></tr>
      <tr><td style='padding:8px; background:#f1f3f5;'><strong>Vai trò</strong></td><td style='padding:8px;'>{roleName}</td></tr>
    </table>
